...
import vn.edu.stu.tranthanhsang.healthy_app.databinding.FragmentHeightBinding
...
class HeightFragment : Fragment() {
    ...
    private fun addEvents() {
        binding.btnNext.setOnClickListener {
            val height = binding.edtScale.text.toString()
            profileViewModel.updateHeight(height.toInt())
    ...
    private fun validateInput(text: String) {
        val heightInput = text.trim().toIntOrNull() // Chuyển thành số
        if (heightInput == null || text.trim().length > 3) { // Kiểm tra số hợp lệ
            binding.btnNext.disable()
            binding.tvError.show()
            binding.tvError.text = getString(R.string.error_height_length)
            return
        }
        when{
            heightInput < 80 -> {
